...
void setLenAndNum(char * dir){
	...
	lenOfLongestFile += 3;		//just for margin
	closedir(dp);}

void showLongList(char * dir, char * fileName){
	//concatenating path
	char temp[100];
    snprintf(temp, 100, "%s/%s", dir, fileName);
...
void do_ls(char * dir){
	...
	if(longList != 1){
		...
		char temp[lenOfLongestFile];
		for(int k=0; k<totalNumOfFiles; k++){
			for(int j=0; j<totalNumOfFiles-1-k; j++){
		...
		printf("\033[%dB \033[1G", numOfRows);	//move the curser to the end of file listing
	}
